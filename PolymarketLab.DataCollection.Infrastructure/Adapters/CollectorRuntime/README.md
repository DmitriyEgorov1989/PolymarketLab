...
этой session ожидаемо не сохраняются. В работающем процессе `CollectorScheduler`
повторяет запрет запуска, ожидает завершения сборщика и атомарно удаляет набор данных,
сохраняет аудит очистки и выполняет `Invalidating -> Failed`. Ошибка остановки не
разрешает очистку; запись заблокированного сборщика сохраняется до `Completion`,
поэтому повторная остановка не принимает тайм-аут за подтверждённое завершение.
Ошибка прохода планировщика журналируется, следующий проход повторяет операцию
в новой области зависимостей. Очередь исходных сообщений и нормализатор продолжают
работать: постоянный запрет записи защищает удалённые данные от поздних сообщений.
Если процесс завершился раньше, очистку выполняет восстановление при следующем
запуске, до запуска ingestion, normalizer и API.
...
9. Без запрета запуска ошибка Stop удаляет entry и допускает replacement. После `FenceSession` entry сохраняется до `Completion`; повторный Start запрещён.
