...
После успешной регистрации backend также гарантирует наличие долговечного задания
сбора для этого рынка. Отдельный `POST /api/Collector` для первоначального задания
не требуется. Ответ сохраняет прежнюю форму `marketId`/`created`; состояние задания
читается через collector endpoints.
...
Если предыдущий запрос сохранил Market, но не смог сохранить задание, повторная
