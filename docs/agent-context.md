...
- Lifecycle scheduler раз в секунду обнаруживает сохранённые active session IDs и запускает не более одной tracked операции на session, каждую в отдельном DI scope. Медленный Gamma/WebSocket start одного рынка не задерживает discovery и подготовку остальных. До `T-60s` каждая session остаётся `Scheduled`, затем exact Gamma boundary check и CAS запускают preparation; временные transport-ошибки повторяются до `T = EventStartsAt`, включая период после `T-10s`, а protocol/identity/readiness persistence errors немедленно инвалидируют попытку. Readiness принимается только строго до `T` после durable enqueue initial `book` каждого snapshot token и matching `PONG` текущей connection epoch; connect/subscription недостаточны. Новая connection epoch сбрасывает readiness в `AwaitingInitialBooks`, не используя observations прежней epoch. Missed window завершается failure и cleanup.
...
- После durable raw equality session сохраняет `AwaitingNormalizationAt` и остаётся `Stopping/AwaitingNormalization` максимум до `AwaitingNormalizationAt + 5m`. Один PostgreSQL statement доказывает exact Processed cardinality snapshot `ProjectionVersion` и strict WS `market_resolved` provenance. Перед terminal CAS committed typed order-book events той же session/version воспроизводятся в ephemeral states по `(raw_message_id, raw_item_index)`; каждый token immutable session snapshot обязан иметь committed initial `book`, а проверяемое нарушение целостности инвалидирует и очищает только эту session с `PersistenceFailure`. `Pending`, `Processing` и missing snapshot rows ожидаются до deadline; `Invalid`, `Unsupported`, terminal `Failed`, runtime version mismatch, legacy null version, provenance mismatch, отсутствующий initial `book`, integrity issue и timeout запускают durable invalidation через `ICollectorSessionInvalidationCoordinator`. Deadline повторно проверяется после integrity gate, непосредственно перед terminal CAS; только успешный gate строго до deadline CAS-выполняет `Stopped/MarketClosed`. Empty root array может быть `Processed` с нулём normalized events, а дополнительные ledger/projection rows других версий не влияют на доказательство snapshot-версии. Market WebSocket не предоставляет пригодного session sequence number, поэтому gate не обещает обнаружить каждый пропуск внешнего события; transport discontinuity после readiness по-прежнему инвалидирует session.
