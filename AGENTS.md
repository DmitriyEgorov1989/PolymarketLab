...
- Когда пользователь просит изучить, взять или спланировать задачу до реализации, обязательно вызови навык `analyzing-tasks`. Сначала объясни назначение задачи простыми словами и приведи конкретные примеры успешного, ожидающего и ошибочного сценариев. Для числовых значений явно указывай смысл и единицы, например цену `0.01`, вероятность `1%` или terminal settlement `1.00`.
...
- Для C# моделей и интерфейсов добавляй содержательные XML-комментарии на русском языке к типам и членам, включая семантику `null`.
